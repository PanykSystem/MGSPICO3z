...
{
	spi_init( MMC_SPI_DEV, 10 * 1000 * 1000 ); 		/* 10Mbps */
...
)
{
    ...
    src = dat;
    spi_write_read_blocking( MMC_SPI_DEV, &src, &dst, 1 );
    return  (BYTE)dst;
}
...
)
{
    ...
    spi_read_blocking( MMC_SPI_DEV, 0xff, buff, btr );
}
...
{
    spi_write_blocking( MMC_SPI_DEV, buff, btx );
}
