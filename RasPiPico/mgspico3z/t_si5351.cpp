...
{
	i2c_init(SI5351_I2C_DEV, I2C_BAUDRATE);
	gpio_set_function(SI5351_I2C_PIN_SDA, GPIO_FUNC_I2C);
	gpio_set_function(SI5351_I2C_PIN_SCL, GPIO_FUNC_I2C);
	gpio_pull_up(SI5351_I2C_PIN_SDA);
	gpio_pull_up(SI5351_I2C_PIN_SCL);
	busy_wait_ms(10);
	return;
}
...
void sendReg8(const uint8_t reg, const uint8_t dt)
{
	...
	i2c_write_blocking(
		SI5351_I2C_DEV, SI5351_I2C_ADDR, dts, sizeof(dts),	false);
	busy_wait_us(WAITV);
	return;
}

static void setup_Si5351_CLK0_3072000_Hz()
{
...
void t_SetupSi5351()
{
	...
	// // CLK0 for SPDIF
	setup_Si5351_CLK0_3072000_Hz();
	// CLK1 for OPLL,PSG,IKASCC,TV80
	setup_Si5351_CLK1__3579545_Hz();

	// Reg.3 CLK0, CLK1の出力をEnbale
	sendReg8(3, 0xfc);

	i2c_deinit(SI5351_I2C_DEV);
	return;
