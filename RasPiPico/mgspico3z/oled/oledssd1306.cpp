...
{
	i2c_init(SSD1306_I2C_DEV, I2C_BAUDRATE);
...
void CSsd1306I2c::initSSD1306()
{
	...
	i2c_write_blocking(
		SSD1306_I2C_DEV, SSD1306_I2C_ADDR,
		buff, sizeof(buff),
		false);
	return;
};
...
void CSsd1306I2c::FillBox(int sx, int sy, int lx, int ly, bool bDot)
{
	for( int t = 0; t < sy+ly-1; ++t){
		Line(sx, sy+t, sx+lx, sy+t, bDot);
	}
	return;
}
